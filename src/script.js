...
const clickOnCheckbox = async (index) => {
	let {_id, text, isCheck, __v} = taskListArray[index];
	const response = await fetch (`http://localhost:8000/updateTask?_id=${taskListArray[index]._id}`,{
		method: 'PATCH',
		headers: {
			'Content-Type' : 'application/json;charset=utf-8',
			'Access-Control-Allow-Origin' : '*'
...
const saveChangesInInput = async (index) => {
	...
	if (!isEmpty(input.value)) {
		if (confirm('There is no value in the input field. Are you sure?')) {
			const response = await fetch(`http://localhost:8000/deleteTask?_id=${taskListArray[index]._id}`, {
				method : 'DELETE',
				headers : {
					'Access-Control-Allow-Origin' : '*'
				}
